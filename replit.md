...
**سبب المشكلة**: على شبكة 4G يمنية، صورة كاميرا الجوال = 3-5 MB ورفعها يأخذ 30-50 ثانية. الـ frontend timeout (15s) كان يُلغي الطلب **قبل** وصوله للسيرفر، فيظهر "انتهت مهلة الاتصال" رغم أن الإنترنت يعمل.

### الإصلاحات في `client/src/components/Navbar.tsx`:
1. **`compressImageInBrowser()` جديدة** — تضغط الصورة عبر Canvas API قبل الرفع:
   - أبعاد قصوى 800px (يحافظ على دقة Gemini كاملة)
   - JPEG quality 0.82
   - النتيجة: من 5MB → ~120KB = **رفع أسرع 30-50×**
   - توفير ضخم في باقة بيانات المستخدم (مهم في اليمن)
2. **رفع `timeout` من 15ث → 45ث** — يكفي حتى لأبطأ الشبكات
3. **رفع حد الصورة من 5MB → 20MB** — لأن الضغط يحدث في المتصفح
