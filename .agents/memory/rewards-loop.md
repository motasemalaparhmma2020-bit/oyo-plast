...
**Referral payout amount is gated ONLY by `referral_enabled`** — the YER amount must never gate it. Resolve the amount through one shared helper that returns the stored `referral_reward_yer` only when it is a finite value > 0, else a non-zero default (mirrors the `friend_discount || 15` convention); guard against 0/NULL **and negative** so a bad config can't pay a negative wallet credit. **Why:** "activating" referral means enabling it; if the amount also gated payout, a store that enabled referral but left the amount at 0 (or negative) would silently pay nothing/garbage. **How to apply:** call the same helper in both the payout logic and the `/api/referral/me` display so they can't drift, and bootstrap existing `<= 0`/NULL stores to the default in the startup migration.
