...
/* ============================================================
   ...
   RTL • Cairo • Sky blue #2196F3 • محاكاة AI
   ============================================================ */
...
export default function ProductPreviewMockup() {
  ...
  const [printColor, setPrintColor] = useState(PRINT_COLORS[1]);
    ...
    <div dir="rtl" className="min-h-screen w-full bg-slate-100" style={{ fontFamily: "Cairo, Tajawal, sans-serif" }}>
      <div className="mx-auto max-w-[480px] bg-slate-50 pb-28 shadow-xl">
        <header className="sticky top-0 z-30 flex items-center gap-2 border-b border-slate-200 bg-white/95 px-3 py-2.5 backdrop-blur">
          ...
          <button className="relative grid h-9 w-9 place-items-center rounded-full hover:bg-slate-100">
        ...
        <section className="relative bg-white">
          <div className="relative aspect-square w-full overflow-hidden bg-gradient-to-b from-slate-50 to-slate-100">
            <img src={mainImage} alt="المنتج" className="h-full w-full object-cover transition-all duration-500" />
            {preview === "fast" && (
              <div className="pointer-events-none absolute inset-0">
                <div className="absolute left-1/2 top-[42%] aspect-square w-[34%] -translate-x-1/2 -translate-y-1/2"
                  style={{
            ...
            <div className="absolute bottom-3 left-3 flex flex-col gap-2">
              <button onClick={() => setWish(!wish)} className="grid h-9 w-9 place-items-center rounded-full bg-white/90 shadow">
          ...
          <div className="flex items-center gap-2 overflow-x-auto px-3 py-2.5">
            {[
              ...
              <button key={t.k} onClick={() => (t.k === "fast" && preview !== "fast" ? runFast() : setPreview(t.k as any))}
        ...
        <section className="bg-white px-4 pb-4">
          <div className="mb-1 flex items-end gap-2">
            <span className="text-2xl font-extrabold" style={{ color: SKY }}>{yer(pieceNow)} ر.ي</span>
          ...
          <h1 className="text-[17px] font-extrabold leading-snug text-slate-800">كيس بلاستيك مطبوع حسب الطلب — جودة فاخرة</h1>
          <div className="mt-1.5 flex items-center gap-2 text-xs text-slate-500">
            <span className="flex items-center gap-0.5 font-bold text-amber-500"><Star className="h-3.5 w-3.5 fill-amber-400 text-amber-400" /> 4.8</span>
            <span>(320 تقييم)</span>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <Field label="عدد الأوجه" value={faces === 2 ? `وجهان (+${yer(FACE_SETUP)} ر.ي تجهيز)` : "وجه واحد (مجاني)"}>
              <div className="flex gap-2">
                {[1, 2].map((f) => (
                  <button key={f} onClick={() => setFaces(f as 1 | 2)}
            ...
            <div className="mt-3">
              ...
              <div className="flex flex-wrap gap-2">
                {PRINT_COLORS.map((c) => (
                  <button key={c.id} onClick={() => setPrintColor(c)}
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              <Stepper value={designColors} min={1} max={4} onChange={setDesignColors} />
            </div>
          </div>
              ...
              <>
                <div className="mb-3 flex rounded-xl bg-slate-100 p-1">
                  {[
                    ...
                    <button key={t.k} onClick={() => setDesignTab(t.k as any)}
                ...
                {designTab === "upload" ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <ChatInput placeholder="اسم المتجر" value={txt.shop} onChange={(v) => setTxt({ ...txt, shop: v })} />
                    <ChatInput placeholder="رقم الهاتف" value={txt.phone} onChange={(v) => setTxt({ ...txt, phone: v })} />
                    <ChatInput placeholder="العنوان" value={txt.addr} onChange={(v) => setTxt({ ...txt, addr: v })} />
                    <ChatInput placeholder="نشاط المتجر (مثال: مقهى)" value={txt.activity} onChange={(v) => setTxt({ ...txt, activity: v })} />
                    <button onClick={mergeText} className="w-full rounded-xl py-2.5 text-sm font-bold text-white" style={{ background: SKY }}>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <button onClick={runFast} disabled={busy}
                  ...
                  <button onClick={() => runStudio()} disabled={busy}
                ...
                <div className="mt-4">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {ALTS.map((a) => (
                      <button key={a.id} onClick={() => runStudio(a.img)} disabled={busy}
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 mx-auto max-w-[480px] border-t border-slate-200 bg-white/95 px-3 py-2.5 backdrop-blur">
        <div className="mb-1.5 flex items-center justify-between text-[11px] text-slate-500">
          ...
          <span className="text-base font-extrabold" style={{ color: SKY }}>{yer(grandTotal)} ر.ي</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2.5">
            <Layers className="h-4 w-4 text-slate-500" />
            <span className="text-sm font-extrabold text-slate-800">{bundle.count}</span>
          </div>
      ...
      {toast && (
        <div className="fixed bottom-24 left-1/2 z-50 -translate-x-1/2 rounded-full bg-slate-900 px-4 py-2 text-xs font-bold text-white shadow-lg">
          {toast}
        </div>
      )}
    </div>
...
function Stepper({ value, min, max, onChange }: { value: number; min: number; max: number; onChange: (n: number) => void }) {
  return (
    <div className="flex items-center gap-1 rounded-xl border border-slate-200 bg-white p-1">
      <button onClick={() => onChange(Math.max(min, value - 1))} className="grid h-8 w-8 place-items-center rounded-lg hover:bg-slate-100">
        <Minus className="h-4 w-4 text-slate-600" />
      </button>
      <span className="w-7 text-center text-sm font-extrabold text-slate-800">{value}</span>
      <button onClick={() => onChange(Math.min(max, value + 1))} className="grid h-8 w-8 place-items-center rounded-lg hover:bg-slate-100">
...
function ChatInput({ placeholder, value, onChange }: { placeholder: string; value: string; onChange: (v: string) => void }) {
  return (
