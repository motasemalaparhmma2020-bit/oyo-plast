...
export function SalesChat({ productId, productName }: SalesChatProps) {
  const [open, setOpen] = useState(false);
  const [messages, setMessages] = useState<Msg[]>([
    {
      role: "model",
      text: productName
        ? `أهلاً وسهلاً! أنا مساعد مبيعات أويو بلاست. أرى أنك مهتم بـ "${productName}" — كم قطعة تحتاج؟ وهل تريد طباعة مخصصة؟`
        : "حياك الله! أنا مساعد مبيعات أويو بلاست. كيف أقدر أساعدك اليوم؟",
    },
  ]);
